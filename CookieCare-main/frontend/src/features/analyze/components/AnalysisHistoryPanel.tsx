...
import { X, Clock, ChevronRight, AlertCircle, FileSearch, Trash2 } from "lucide-react";
                ...
                {history.map((item) => (
                  <li key={item.jobId}>
                    <div className="group flex w-full items-start gap-3 px-5 py-3.5 transition-colors hover:bg-[#F7F8FB]">
                      <button
                        type="button"
                        disabled={loadingSession || deletingId === item.jobId}
                        onClick={() => { setConfirmId(null); onSelectSession(item); }}
                      ...
                      {/* Delete / confirm area — always visible */}
                      {confirmId === item.jobId ? (
                        <div className="mt-1 flex shrink-0 items-center gap-1">
                          <button
                            ...
                            onClick={async (e) => {
                              ...
                              setConfirmId(null);
                              if (onDeleteEntry) await onDeleteEntry(item.jobId);
                              setDeletingId(null);
                            }}
                        ...
                        <button
                          type="button"
                          disabled={deletingId === item.jobId || loadingSession}
                          onClick={(e) => { e.stopPropagation(); setConfirmId(item.jobId); }}
