...
import { MOCK_FINDINGS, MOCK_RECOMMENDATIONS } from "../constants";
import { DPARadialGauge }  from "./DPARadialGauge";
import { DPAFindingCard }  from "./DPAFindingCard";

// MOCK_RECOMMENDATIONS is defined in constants.ts (DPA-specific)
// Re-export the inline type so we stay DRY
...
export function DPAResultsState({ fileName, onReset }: DPAResultsStateProps) {
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2.5">
                {filteredFindings.map((finding) => (
                  <DPAFindingCard key={finding.id} finding={finding} />
                ))}
              </div>
            </div>
          )}
