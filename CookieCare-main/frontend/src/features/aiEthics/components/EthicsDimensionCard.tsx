import type { EthicsDimensionResult } from "../utils/categorizeFindings";

interface EthicsDimensionCardProps {
  result: EthicsDimensionResult;
  selected: boolean;
  onSelect: () => void;
  index: number;
}
...
const STATUS_STYLES = {
  strong: { badge: "bg-badge-green text-badge-green-text", bar: "#3D9B8F", icon: "/icons/check.svg" },
  partial: { badge: "bg-badge-yellow text-badge-yellow-text", bar: "#C9843A", icon: "/icons/warning.svg" },
  critical: { badge: "bg-badge-red text-badge-red-text", bar: "#B54A45", icon: "/icons/cross.svg" },
} as const;

export function EthicsDimensionCard({ result, selected, onSelect, index }: EthicsDimensionCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className="group flex h-full min-h-[228px] cursor-pointer flex-col rounded-[22px] bg-white p-5 text-left transition-[transform,box-shadow] duration-200 hover:-translate-y-px"
      style={{
    ...
    >
      ...
      <div className="mt-5">
        <div className="mb-2 flex items-baseline justify-between">
          <span className="text-[10px] font-medium uppercase tracking-[0.14em] text-[#98A2B3]">Score</span>
        ...
        <div className="h-[5px] overflow-hidden rounded-full bg-[#F2F4F7]">
          <div
            ...
            style={{ width: `${Math.max(score, 3)}%`, background: styles.bar }}
          />
        </div>
      </div>
      <div className="mt-4 flex items-center justify-between gap-2 pt-1">
        ...
        <span className={`text-[12px] font-medium ${selected ? "text-[#4F5BD9]" : "text-[#98A2B3] group-hover:text-[#4F5BD9]"}`}>
