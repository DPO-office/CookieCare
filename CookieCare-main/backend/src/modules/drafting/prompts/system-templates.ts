...
- Structure: Start with a clear legal document title derived from the Contract Type (e.g. "MUTUAL NON-DISCLOSURE AGREEMENT" for NDA — never a marketing label, party nickname, or UI field like "Vendor Infrastructure Host"). Do NOT include a Table of Contents. Use the compulsory skeleton section names in order. Add a footer indicating the document is confidential.
...
- Structure: Include a professional title page when appropriate. Do NOT generate a Table of Contents under any circumstances. Use standard legal section names (e.g., "Term and Termination" instead of "Termination & Survival Mechanics"). Add page numbers and a footer indicating the document is confidential. Include schedules or annexures when referenced.
- Quality: Use consistent defined terms throughout the agreement. Avoid repetitive wording and unnecessary repetition. Ensure all cross-references point to the correct clauses. Remove redundant provisions. Ensure clause numbering is sequential and accurate. Verify that all mandatory clauses are present for the selected agreement type.
