...
export function groundFindings(state: AnalysisState): AnalysisState {
  ...
  for (const finding of findings) {
    ...
    // ACT-Phase 9 — a `verifiedByProposition` finding was already checked
    // against its EXACT candidate passage by VERIFY's own deterministic
    // quote check (verify-proposition.ts's quoteAppearsIn) before it was
    // ever allowed to carry status="present" — a stronger, narrower
    // guarantee than this whole-document re-search provides. Re-running it
    // here is exactly "re-doing grounding work ACT should never have
    // skipped in the first place" (research doc §2.1); skip it.
    if (finding.status === "present" && !finding.verifiedByProposition && !quoteInSource(state, finding)) {
