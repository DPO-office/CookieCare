...
describe("deriveReportOutline", () => {
  it("splits Art 28 particulars and mandatory clauses into separate analysis items", () => {
    ...
    const outline = deriveReportOutline(intent, "regime_compliance_memo", "deep");
  ...
  it("creates additional analysis items for remaining requirements", () => {
    ...
    const outline = deriveReportOutline(intent, "regime_compliance_memo", "standard");
    const analysis = analysisItems(outline);

    // We should have at least one item that includes `some_other_requirement`.
