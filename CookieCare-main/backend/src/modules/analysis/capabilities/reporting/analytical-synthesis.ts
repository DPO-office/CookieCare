...
import { getAnalysisProfile, profileThinkingLevel } from "../../utils/profile-thinking.js";
...
function guardSynthesis(
  ...
  return {
    ...
    keyThemes: themes.slice(0, maxItems),
    substantiveVsDrafting: String(raw.substantiveVsDrafting ?? "").trim(),
    materialRisks: materialRisks.slice(0, maxItems),
...
export async function runAnalyticalSynthesis(
  ...
  const thinkingMode = getAnalysisProfile(state).thinkingMode;
  try {
    const raw = await executeJsonCompletion<AnalyticalSynthesis>(
      ...
      LLMProvider.GEMINI,
      {
        ...
        // ACT-Phase 10 — resolve-synthesis-ceiling.ts already existed for
        // exactly this (profile hard cap + report complexity + truncation-
        // repair bump) but this call was never wired to it; ACT-Phase 7
        // papered over the resulting truncation with a hardcoded 3000,
        // which was actually below the Lite hard cap this should have used.
        // Falls back to the flat profile hard cap when no reportSpec exists
        // yet (this path can run before PLAN's reportSpec is set).
        maxOutputTokens: state.plan?.reportSpec
          ? resolveSynthesisMaxOutputTokens(state, state.plan.reportSpec)
          : getAnalysisProfile(state).synthesisHardCap,
      }
    );
    ...
    return guardSynthesis(raw, assessments, factRollup, thinkingMode === "lite" ? 4 : 8);
  } catch (err) {
