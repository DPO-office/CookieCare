 ...
 * `verifiedByProposition` so this has zero effect on any other finding path
 * — those keep their existing, unchanged status-only rule.
 */
function isSupporting(f: Finding): boolean {
  if (f.status !== "present") return false;
  if (f.verifiedByProposition && f.judgement?.nli !== "entailed") return false;
  return true;
}
