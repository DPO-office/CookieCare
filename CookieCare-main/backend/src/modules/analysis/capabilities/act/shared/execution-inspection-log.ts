...
import { actStageForTool, type ActStage } from "./execution-stage.js";
...
export function logActGraphInspect(state: AnalysisState, runnable: AnalysisWorkUnit[]): void {
  ...
  const lines: string[] = [
    ...
    `   stages   INVESTIGATE (candidate gen) → VERIFY (proves/gap) → LOCK (aggregate) → RENDER (format)`,
    "",
  ];
  const byStage = countBy(runnable, (u) => actStageForTool(u.tool));
  lines.push(`   stage mix  ${fmtCounts(byStage)}`, "");
  runnable.forEach((unit, i) => {
    lines.push(
      `   ${i + 1}. [${actStageForTool(unit.tool)}] ${unit.tool}  id=${unit.workUnitId}  deps=${unit.dependsOn.join(",") || "(none)"}`
    );
...
export function logActStepInspect(args: ActStepInspectArgs): void {
  ...
  const { unit, state, priorState, emitted, ms, stepIndex, stepTotal, failed, error } =
    args;
  const stage = actStageForTool(unit.tool);
  const lines: string[] = [
    `STEP ${stepIndex}/${stepTotal}  [${stage}]  tool=${unit.tool}  id=${unit.workUnitId}  ms=${ms}${failed ? "  FAILED" : ""}`,
  ...
  pacLogBlock(`ACT STEP — [${stage}] ${unit.tool}`, lines);
}
...
function formatStageRollup(units: AnalysisWorkUnit[]): string[] {
  const stages: ActStage[] = ["INVESTIGATE", "VERIFY", "LOCK", "RENDER"];
  const lines: string[] = [
    "0. STAGE ROLLUP  (INVESTIGATE → VERIFY → LOCK → RENDER)",
  ];
  for (const stage of stages) {
    const group = units.filter((u) => actStageForTool(u.tool) === stage && stage !== "SETUP");
    if (stage === "INVESTIGATE") {
      // SETUP (classify_document) has no separate stage row of its own —
      // fold its units into the count so nothing silently disappears.
      const setupUnits = units.filter((u) => actStageForTool(u.tool) === "SETUP");
      group.push(...setupUnits);
    }
    const emitted = group.reduce((n, u) => n + (u.findingsEmitted ?? 0), 0);
    const failed = group.filter((u) => u.status === "failed").length;
    const tools = summarizeTools(group);
    lines.push(
      `   ${stage.padEnd(11)} units=${group.length}  findings=${emitted}  failed=${failed}${tools ? `  (${tools})` : ""}`
    );
  }
  return lines;
}
