...
import { tokenizeForEvidence } from "../../shared/requirement-evidence.js";
import { buildSectionCandidates, inferEvidenceRelationshipScope } from "../../shared/candidate-selection.js";
...
export interface Phase3Hit {
  ...
  selectedBy: string[];
  score: number;
  /** Graph-native semantic-review diagnostics. */
  reviewDisposition?:
    | "accepted"
    | "semantic_rejection"
    | "role_budget"
    ...
    | "not_classified";
  evidenceRole?: "primary" | "supporting" | "dependency" | "definition" | "limitation" | "contradictory" | "context" | "irrelevant";
...
export interface Phase3RecallSummary {
  ...
  totalQueries: number;
  /** Primary-proof coverage is separate from mere candidate availability. */
  proofElementsCovered?: string[];
  proofElementsUnresolved?: string[];
  acceptedCandidateCount?: number;
  semanticRejectedCandidateCount?: number;
  budgetOmittedCandidateCount?: number;
...
export interface Phase3BundleItem {
  ...
  /** Investigation-v2 metadata; ignored by existing Phase 4 verification. */
  evidenceRole?: "primary" | "supporting" | "dependency" | "definition" | "limitation" | "contradictory" | "context";
  contributesToElementIds?: string[];
...
export interface Phase3Bundle {
  ...
  /** Explicit completeness replaces the legacy synthetic dependency marker. */
...
export interface Phase3PerRequirementResult {
  ...
  bundle: Phase3Bundle;
  /**
   * Which pipeline actually produced each field group, so a shadow-mode dump
   * never implies `queries`/`hits`/`expansions` and `bundle` came from the
   * same run when one is legacy-carried and the other graph-native.
   */
  provenance?: {
    queries: "legacy" | "graph_native";
    hits: "legacy" | "graph_native";
    expansions: "legacy" | "graph_native";
    bundle: "legacy" | "graph_native";
  };
}
...
/**
 * Compute Phase 3 evidence bundles for a batch of requirements over one
