...
export function buildEvaluatePackageUserPrompt(input: {
  ...
  return [
    ...
    "- strong: the quote fully substantiates the required element with operative detail (not a heading or annex pointer).",
    "- adequate: the required element is present and verifiable, but the quote is thinner or formulaic.",
    "- conditional: some required elements are present and others are absent, incomplete, annex-dependent, or too weak to verify adequacy.",
    ...
    "If evidence is marked status=referenced_elsewhere, or the quote refers to an annex, schedule, SOW, appendix, exhibit, incorporated policy, or another agreement that is not in this evidence set, use cannot_determine (not gap) and name the referenced document in the rationale.",
    "Distinguish existence of an obligation from whether its adequacy can be verified. If the obligation is mentioned but the operative detail lives elsewhere, use cannot_determine or conditional — not gap.",
