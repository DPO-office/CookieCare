...
/** Absolute path to the skills content root (skills/). */
...
/** Resolve `{skillId}/SKILL.md` under the skills root. */
export function skillMarkdownPath(skillId: string): string {
  return path.join(SKILLS_ROOT, skillId, "SKILL.md");
}
...
export async function loadSkillMarkdown(skillId: string): Promise<string> {
  try {
    return await readFile(skillMarkdownPath(skillId), "utf8");
  } catch {
    return "";
  }
}
