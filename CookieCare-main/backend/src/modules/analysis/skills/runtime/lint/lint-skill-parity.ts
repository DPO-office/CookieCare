...
export function lintSkillParity(): SkillParityViolation[] {
  ...
  for (const skill of Object.values(registry)) {
    const mdPath = skillMarkdownPath(skill.skillId);
  ...
  const manifestIds = new Set(skillManifest.map((e) => e.skillId));
  for (const skillId of registryIds) {
    if (!manifestIds.has(skillId)) {
      violations.push({
        skillId,
        kind: "missing_manifest_entry",
        detail: `Registry skill "${skillId}" has no skillManifest entry`,
      });
    }
  }
  for (const skillId of manifestIds) {
    if (!registryIds.has(skillId)) {
      violations.push({
        skillId,
        kind: "orphan_manifest_entry",
        detail: `skillManifest lists "${skillId}" but it is not in the registry`,
      });
    }
  }
