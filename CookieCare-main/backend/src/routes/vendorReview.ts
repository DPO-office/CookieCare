 ...
 *   - files[]  : one or more vendor documents (PDF, DOCX, TXT) — optional
 *   - vendorUrl: vendor website URL — optional
 *
 * At least one of files or vendorUrl must be provided.
 *
 * Extracts text server-side, queues a vendor_review background job, and
 * returns a job_id so the frontend can subscribe via SSE (/api/jobs/sse).
 *
 * Follows the same pattern as /api/dpa/review.
 */
  ...
  async (req: Request, res: Response) => {
    ...
    if (!hasFiles && !hasUrl) {
      return res.status(400).json({
        error:
          "At least one vendor document or a vendor URL is required. " +
          "Send files in a 'files' multipart field and/or a 'vendorUrl' text field.",
      });
    }
    ...
    if (hasFiles) {
      ...
      if (!hasUrl && extractedTexts.length === 0) {
        return res.status(422).json({
          error:
            "All uploaded files appear to be empty or contain insufficient text for analysis.",
        });
      }
    }
    ...
    // ── Queue the vendor_review job ────────────────────────────────────────
    try {
      const job = await addJobToQueue(req.user!.id, "vendor_review", {
        documentText: mergedDocumentText,
        vendorUrl:    hasUrl ? vendorUrl : undefined,
        fileNames,
      });

      console.log(
        `[vendor-review] Queued vendor_review job ${job.id} for user ${req.user!.id} — ` +
          `files: ${fileNames.join(", ") || "none"}, url: ${vendorUrl || "none"}, ` +
